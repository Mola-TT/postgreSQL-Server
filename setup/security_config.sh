...
# Additional rate limiting maps
map $http_user_agent $rate_limit {
    default "";
    ~*(bot|crawler|spider|scraper) $binary_remote_addr;
    ~*(wget|curl|python|php|perl) $binary_remote_addr;
}

# Block based on user agent
map $http_user_agent $blocked_agent {
    default 0;
    ~*(nikto|nmap|sqlmap|w3af|acunetix|nessus|openvas) 1;
    ~*(masscan|zmap|dirb|dirbuster|gobuster|hydra) 1;
}
...
setup_security() {
    ...
    if [ "$nginx_security_success" = true ]; then
        nginx -t >/dev/null 2>&1 && systemctl reload nginx >/dev/null 2>&1 || log_warn "Failed to reload nginx"
    fi
