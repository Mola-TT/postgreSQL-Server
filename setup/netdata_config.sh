...
configure_netdata() {
  ...
  # Configuration to disable direct access and use Nginx as a proxy
  {
    ...
    echo "# Generated by netdata_config.sh"
    echo ""
    echo "[global]"
    echo "  hostname = ${NETDATA_HOSTNAME:-PostgreSQL Server}"
    echo ""
    echo "[web]"
    echo "  mode = none" # Disable direct web UI access
    echo "  allow netdata from = localhost fd* 127.* 10.* 192.168.* 172.16.* 172.17.* 172.18.* 172.19.* 172.20.* 172.21.* 172.22.* 172.23.* 172.24.* 172.25.* 172.26.* 172.27.* 172.28.* 172.29.* 172.30.* 172.31.*"
    echo "  bind to = 127.0.0.1"
    echo "  allow connections from = localhost"
    echo "  disconnect idle clients after seconds = 3600"
    echo "  timeout for first request = 60"
    echo ""
  } > "$netdata_conf_dir/netdata.conf"
  ...
  {
    echo "# Netdata health notification configuration"
    echo "# Generated by netdata_config.sh"
    echo ""
    echo "SEND_EMAIL=\"YES\""
    echo "EMAIL_SENDER=\"${NETDATA_EMAIL_SENDER:-netdata@localhost}\""
    echo "DEFAULT_RECIPIENT_EMAIL=\"${NETDATA_EMAIL_RECIPIENT:-root}\""
    echo ""
    echo "# The SMTP server configuration"
    echo "SMTP_SERVER=\"${NETDATA_SMTP_SERVER:-localhost}\""
    echo "SMTP_PORT=${NETDATA_SMTP_PORT:-25}"
    echo "SMTP_TLS=\"${NETDATA_SMTP_TLS:-YES}\""
    echo "SMTP_USERNAME=\"${NETDATA_SMTP_USERNAME:-}\""
    echo "SMTP_PASSWORD=\"${NETDATA_SMTP_PASSWORD:-}\""
    echo ""
  } > "$health_conf_dir"
  ...
  # CPU usage alarm
  {
    echo "# CPU usage alarm at 80%"
    echo "# Generated by netdata_config.sh"
    echo ""
    echo "alarm: cpu_usage_80"
    echo "      on: system.cpu"
    echo "      os: linux"
    echo "  lookup: average -1m percentage of user,system,softirq,irq,guest"
    echo "   units: %"
    echo "   every: 1m"
    echo "    warn: $this > 80"
    echo "    crit: $this > 90"
    echo "   delay: up 0 down 5m"
    echo "    info: CPU utilization over 80%"
    echo " options: no-clear-notification"
    echo ""
  } > "$netdata_conf_dir/health.d/cpu_usage.conf"
  
  # Memory usage alarm
  {
    echo "# Memory usage alarm at 80%"
    echo "# Generated by netdata_config.sh"
    echo ""
    echo "alarm: ram_usage_80"
    echo "      on: system.ram"
    echo "      os: linux"
    echo "  lookup: average -1m percentage of used"
    echo "   units: %"
    echo "   every: 1m"
    echo "    warn: $this > 80"
    echo "    crit: $this > 90"
    echo "   delay: up 0 down 5m"
    echo "    info: RAM utilization over 80%"
    echo " options: no-clear-notification"
    echo ""
  } > "$netdata_conf_dir/health.d/ram_usage.conf"
  
  # Disk usage alarm
  {
    echo "# Disk usage alarm at 80%"
    echo "# Generated by netdata_config.sh"
    echo ""
    echo "alarm: disk_usage_80"
    echo "      on: disk.space"
    echo "      os: linux"
    echo "  lookup: average -1m percentage of used"
    echo "   units: %"
    ...
    echo "    crit: $this > 90"
    echo "   delay: up 0 down 5m"
    echo "    info: Disk utilization over 80%"
    echo " options: no-clear-notification"
    echo ""
  } > "$netdata_conf_dir/health.d/disk_usage.conf"
...
configure_nginx_for_netdata() {
  ...
  {
    echo "# Netdata monitoring via Nginx"
    echo "# Generated by netdata_config.sh"
    echo ""
    echo "upstream netdata {"
    echo "    server 127.0.0.1:19999;"
    echo "    keepalive 64;"
    echo "}"
    echo ""
    ...
    echo "    listen 80;"
    echo "    listen [::]:80;"
    echo "    server_name monitor.$domain;"
    echo ""
    echo "    location / {"
    ...
    echo ""
    echo "    # Basic auth protection"
    echo "    auth_basic \"Netdata Monitoring\";"
    echo "    auth_basic_user_file /etc/nginx/.htpasswd.netdata;"
    echo ""
    echo "    # Proxy to Netdata"
    echo "    location / {"
    echo "        proxy_pass http://netdata;"
