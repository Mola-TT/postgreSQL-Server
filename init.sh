...
NETDATA_CONFIG="$SCRIPT_DIR/setup/netdata_config_fix.sh"
if [ -f "$NETDATA_CONFIG" ]; then
    ...
    for alt_path in "/home/tom/postgreSQL-Server/setup/netdata_config_fix.sh" "$(dirname "$SCRIPT_DIR")/setup/netdata_config_fix.sh"; do
        if [ -f "$alt_path" ]; then
    ...
    if [ ! -f "$NETDATA_CONFIG" ] && [ ! -f "$alt_path" ]; then
        ...
        log_error "  - /home/tom/postgreSQL-Server/setup/netdata_config_fix.sh"
        log_error "  - $(dirname "$SCRIPT_DIR")/setup/netdata_config_fix.sh"
    fi
fi
