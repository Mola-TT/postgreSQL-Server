...
test_pgbouncer_connection() {
    ...
    # Try different SSL modes to connect through pgbouncer
    if PGPASSWORD="$PG_SUPERUSER_PASSWORD" psql "host=$host port=$PGB_LISTEN_PORT dbname=$db_name user=postgres sslmode=prefer" -c "SELECT version();" -t --no-align --quiet --tuples-only > /dev/null 2>&1; then
        log_info "✓ PASS: Connected to PostgreSQL through pgbouncer on port $PGB_LISTEN_PORT to database '$db_name'"
        eval "$result_var=true"
        return 0
    else
        local err_output
        err_output=$(PGPASSWORD="$PG_SUPERUSER_PASSWORD" psql "host=$host port=$PGB_LISTEN_PORT dbname=$db_name user=postgres sslmode=prefer" -c "SELECT version();" 2>&1)
...
test_database_connection() {
    if [ "$DB_NAME" != "postgres" ]; then
        ...
        if output=$(PGPASSWORD="${PG_SUPERUSER_PASSWORD}" psql "host=localhost port=${PGB_LISTEN_PORT} dbname=${DB_NAME} user=postgres sslmode=prefer" -c "SELECT 1 as connected;" -t 2>/dev/null); then
            if [[ "$output" == *"1"* ]]; then
...
test_temp_user_connection() {
    ...
    if output=$(PGPASSWORD="$temp_password" psql "host=localhost port=${PGB_LISTEN_PORT} dbname=postgres user=${temp_user} sslmode=prefer" -c "SELECT 'temp_user_connected' as result;" -t 2>/dev/null); then
        if [[ "$output" == *"temp_user_connected"* ]]; then
